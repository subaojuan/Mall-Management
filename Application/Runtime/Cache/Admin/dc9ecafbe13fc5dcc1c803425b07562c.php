...
<li><a  style="text-decoration: none;" ><?php echo ($nav1); ?>     </a></li>
  ...
  <div class="panel-heading">
  	区域列表
    <button class="btn btn-primary navbar-right navbt" ><a href=<?php echo U('area_add');?>  >添加学校</a></button>
  </div>
  ...
  <ul class="list-group" id="cate_ul" >
	<?php if(is_array($area_data)): foreach($area_data as $key=>$vo): if($vo['a_pid'] == 0 && $vo['a_flag'] == 'y' ): ?><li class="list-group-item" style="background-color:#8EBFFA ;">   
     <?php echo ($vo["a_name"]); ?>
      	<?php if(is_array($area_data)): foreach($area_data as $key=>$v1): if($v1['a_pid'] == $vo['a_id'] ): ?><li class="list-group-item" >   
    <span class="badge"><a href=<?php echo U("area_del?id=".$v1['a_id']);?> onclick="return confirm('您确定删除此学校名称?');" >删除</a></span>
    <span class="badge"><a href=<?php echo U("area_update?id=".$v1['a_id']);?>  >编辑</a></span>
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
  <span class="glyphicon glyphicon-flag cons" aria-hidden="true"></span> 
  <?php echo ($v1["a_name"]); ?>
