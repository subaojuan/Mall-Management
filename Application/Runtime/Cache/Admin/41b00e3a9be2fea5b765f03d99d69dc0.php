...
<li><a  style="text-decoration: none;" ><?php echo ($nav1); ?>     </a></li>
  ...
  <div class="panel-heading">
  	添加分类
    <button class="btn btn-primary navbar-right navbt" ><a href=<?php echo U('cat_list');?> >分类列表<a></button>
  </div>
  <div class="panel-body">
<form enctype="multipart/form-data"  action="/index.php/Admin/Categorys/cat_add.html" method="post">
  <ul class="list-group cate_add" id="cate_add" >
     		 
    <li class="list-group-item" > 
    	
    	<label for="exampleInputName2">选择主分类:</label>
    	 <select name="c_pid"  id="sele_cate" class="search-1" >
            <option value="0" >顶级分类</option>
            <?php if(is_array($cate_data)): foreach($cate_data as $key=>$vo): ?><option value="<?php echo ($vo["c_id"]); ?>"  > <?php echo ($vo["c_name"]); ?> </option><?php endforeach; endif; ?>	
             
        </select>
    </li>
    <li class="list-group-item" >
    	<label for="exampleInputName2">分类名称:</label>
    	<input type="text" name="c_name" class="form-control cate_input" id="exampleInputName2" placeholder="请输入分类名称">
