...
<div class="container" style="margin-left: -18px;">
	...
	<div class="panel panel-warning" >
	  ...
	  <form enctype="multipart/form-data"  action="/school/index.php/Home/Goods/goods_update.html?g_id=26" method="post">
	  <ul class="list-group "  >	
	  	<li class="list-group-item" >
	       <label for="exampleInputName2" >商品logo:</label>
	       <div id="preview">
               <img id="imghead" border="0" src="/school/Public<?php echo ($data["g_logo"]); ?>" width="220" height="210" onclick="$('#previewImg').click();">
           </div>         
           <input type="file"  name="g_logo" onchange="previewImage(this)" style="display: none;" id="previewImg">	    	
	    </li>  
	    <li class="list-group-item" >
	   <label for="exampleInputName2">商品名称:</label>
	    <input type="text" name="g_name" class="form-control cate_input" id="" value="<?php echo ($data["g_name"]); ?>">
	    	
	    </li>
	        <li class="list-group-item" >
	    	<label for="exampleInputName2">商品品牌:</label>
	    	<input type="text" name="g_brand" class="form-control cate_input" id="" value="<?php echo ($data["g_brand"]); ?>">
	    	 
	    </li>
	    <li class="list-group-item" >
	    	<label for="exampleInputName2">商品数量:</label>
	    	<input type="text" name="g_num" class="form-control cate_input"  value="<?php echo ($data["g_num"]); ?>">
	    ...
	    <li class="list-group-item" >
	    	...
	    	<select name="g_category"   class="select1" id="select1" >
	          ...
	          <?php if(is_array($cate_data1)): foreach($cate_data1 as $key=>$v1): ?><option value=<?php echo ($v1["c_id"]); ?> 
	            	<?php if($cate_id == $v1['c_id']): ?>selected=selected<?php endif; ?> >
	             <?php echo ($v1["c_name"]); ?>	         		
	            </option><?php endforeach; endif; ?>  
	        </select>
	    	<select name="g_categorys"   class="select1" id="select2" >
	            <option value="" >选择子分类</option> 
	          <?php if(is_array($cate_data2)): foreach($cate_data2 as $key=>$v2): ?><option value=<?php echo ($v2["c_id"]); ?> 
	            	<?php if($cate_ids == $v2['c_id']): ?>selected=selected<?php endif; ?> >
	             <?php echo ($v2["c_name"]); ?>	         		
	            </option><?php endforeach; endif; ?>  
	    ...
	    <li class="list-group-item" >
	    	...
	    	<input type="text" name="g_price" class="form-control cate_input"  value="<?php echo ($data["g_price"]); ?>">
	    	 
	    </li>
	    <li class="list-group-item" >
	    	<label for="exampleInputName2" >新旧程度:</label>
	    	<select name="g_new"   class="select1" id="select1" >
	    	  <?php if(is_array($new_data)): foreach($new_data as $key=>$v1): ?><option value=<?php echo ($v1["gn_id"]); ?>
	            		<?php if($new_id == $v1['gn_id']): ?>selected=selected<?php endif; ?>  >            		
	            		<?php echo ($v1["gn_name"]); ?>
	            	</option><?php endforeach; endif; ?> 
	     ...
	     <li class="list-group-item" >
	       ...
	       <select name="is_price"   class="select1" id="select1" >
	            <option value="1" <?php if($is_id == 1): ?>selected=selected<?php endif; ?> >可议价</option>  
	            <option value="0" <?php if($is_id == 0): ?>selected=selected<?php endif; ?> >不可议价</option> 
	        </select>
	     </li>
	   ...
	   <input type="hidden" name="g_id" value=<?php echo ($data["g_id"]); ?> ></input> 	
